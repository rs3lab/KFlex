...
- IRQ save/restore: [bpf_local_irq_{save,restore} kfuncs](https://lore.kernel.org/bpf/20241204030400.208005-1-memxor@gmail.com). Necessary for spin locks and for implementing an interrupt-safe memory allocator.
- Preemption disable/enable: [bpf_preempt_{disable,enable}](https://lore.kernel.org/bpf/20240424031315.2757363-1-memxor@gmail.com). Necessary for spin locks and for preemption-safe memory allocator. Should typically be combined with IRQ save/restore. BPF programs by default only protect against CPU migration, hence both preemption and IRQ protection are necessary.
